  ...
  return (
    <div className="min-h-screen flex flex-col">
      <main className="w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        {/* Chart Mode Toggle - Outside chart container */}
        ...
        {/* Two column layout: ETF Details + Calculator - Always 3 columns to keep chart position fixed */}
        <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Left column: Chart - Full width in price mode, 2 columns in projection mode */}
          <div className={`flex flex-col ${
            chartMode === "projection" ? "lg:col-span-2" : "lg:col-span-3"
          }`}>
            {/* Chart Section */}
            <div className="bg-primary-500/10 backdrop-blur-sm p-6 rounded-lg shadow-lg border border-primary-500/20 flex-1 flex flex-col min-h-[600px]">
              <ProjectionChart data={growthData} mode={chartMode} etf={selectedETF} />
            </div>
          </div>

          {/* Right column: Prediction Calculator - Only visible in projection mode */}
          {chartMode === "projection" && (
        ...
        {/* Metric Cards Grid - Fixed position below chart, always full width */}
        {selectedETF && (
